...
add_task() {
    ...
    local base_task="{\"id\":$new_id,\"title\":\"$title\",\"assignee\":\"$assignee\",\"created\":\"$timestamp\",\"priority\":\"$priority\"}"

    if [ "$status" = "in-progress" ]; then
        base_task="{\"id\":$new_id,\"title\":\"$title\",\"assignee\":\"$assignee\",\"created\":\"$timestamp\",\"updated\":\"$timestamp\",\"priority\":\"$priority\"}"
    elif [ "$status" = "done" ]; then
        base_task="{\"id\":$new_id,\"title\":\"$title\",\"assignee\":\"$assignee\",\"created\":\"$timestamp\",\"completed\":\"$timestamp\",\"priority\":\"$priority\"}"
    fi

    jq --argjson new_task "$base_task" \
       --arg col "$column_name" \
       '.[$col] += [$new_task]' "$TASKS_FILE" > "$TASKS_FILE.tmp"
...
move_task() {
    ...
    # Extract the task
    local task=$(jq --arg id "$task_id" '[.[].[] | select(.id == ($id | tonumber))][0]' "$TASKS_FILE")
    ...
    # Add to new column
    jq --arg col "$column_name" --argjson task "$task" '.[$col] += [$task]' "$TASKS_FILE" > "$TASKS_FILE.tmp"
    mv "$TASKS_FILE.tmp" "$TASKS_FILE"
